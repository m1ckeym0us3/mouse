...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeMouseAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeMouseAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeMouseAmount = strAnonymizeMouseAmount.remove(strAnonymizeMouseAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeMouseAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeMouseAmount*COIN) nMaxToAnonymize = nAnonymizeMouseAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeMouseAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeMouseAmount));
        strAnonymizeMouseAmount = strAnonymizeMouseAmount.remove(strAnonymizeMouseAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeMouseAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
