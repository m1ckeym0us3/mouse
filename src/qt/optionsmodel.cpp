...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeMouseAmount"))
        settings.setValue("nAnonymizeMouseAmount", 1000);
    nAnonymizeMouseAmount = settings.value("nAnonymizeMouseAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeMouseAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeMouseAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeMouseAmount:
            return QVariant(nAnonymizeMouseAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeMouseAmount:
            nAnonymizeMouseAmount = value.toInt();
            settings.setValue("nAnonymizeMouseAmount", nAnonymizeMouseAmount);
            emit AnonymizeMouseAmountChanged(nAnonymizeMouseAmount);
            break;
        default:
            break;
        }
