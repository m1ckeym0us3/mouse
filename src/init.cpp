...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Mouse.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: moused.pid)") + "\n";
    ...
    strUsage += "  -blockmaxsize=<n>      "   + _("Set maximum block size in bytes (default: 10000)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeharvestamount=<n> " + _("Keep N Mouse anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Mouse is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Mouse is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Mouse version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Mouse server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Mouse") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Mouse to complete") << "\n";
    ...
    nAnonymizeMouseAmount = GetArg("-anonymizeharvestamount", 0);
    if(nAnonymizeMouseAmount > 999999) nAnonymizeMouseAmount = 999999;
    if(nAnonymizeMouseAmount < 2) nAnonymizeMouseAmount = 2;
    ...
    LogPrintf("Anonymize Mouse Amount %d\n", nAnonymizeMouseAmount);
