...
namespace {
    class CMouseAddressVisitor : public boost::static_visitor<bool> {
    private:
        CMouseAddress *addr;
    public:
        CMouseAddressVisitor(CMouseAddress *addrIn) : addr(addrIn) { }
...
bool CMouseAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CMouseAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CMouseAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CMouseAddressVisitor(this), dest);
}

bool CMouseAddress::IsValid() const {
...
CTxDestination CMouseAddress::Get() const {
    if (!IsValid())
...
bool CMouseAddress::GetKeyID(CKeyID &keyID) const {
...
bool CMouseAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CMouseSecret::SetKey(const CKey& vchSecret) {
...
CKey CMouseSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CMouseSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CMouseSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CMouseSecret::SetString(const std::string& strSecret) {
